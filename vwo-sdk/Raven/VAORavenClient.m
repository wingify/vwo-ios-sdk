...
NSString *const kVAORavenLogLevelArray[] = {
    @"debug",
    @"info",
    @"warning",
    @"error",
...
NSString *const VAOuserDefaultsKey = @"nl.mixedCase.RavenClient.Exceptions.vao";
NSString *const VAOsentryProtocol = @"4";
NSString *const VAOsentryClient = @"raven-objc/0.5.0";

static VAORavenClient *sharedClient = nil;

@implementation VAORavenClient

void vaoexceptionHandler(NSException *exception) {
	[[VAORavenClient sharedClient] captureException:exception sendNow:NO];
}
...
- (void)captureMessage:(NSString *)message
    ...
    if (!sendNow) {
        ...
        NSArray *reports = [[NSUserDefaults standardUserDefaults] objectForKey:VAOuserDefaultsKey];
        if (reports != nil) {
            NSMutableArray *reportsCopy = [reports mutableCopy];
            [reportsCopy addObject:data];
            [[NSUserDefaults standardUserDefaults] setObject:reportsCopy forKey:VAOuserDefaultsKey];
        } else {
            reports = [NSArray arrayWithObject:data];
            [[NSUserDefaults standardUserDefaults] setObject:reports forKey:VAOuserDefaultsKey];
        }
...
- (void)setupExceptionHandler {
    NSSetUncaughtExceptionHandler(&vaoexceptionHandler);
    ...
    if (reports != nil && [reports count]) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:[NSArray array] forKey:VAOuserDefaultsKey];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
}
...
- (NSDictionary *)prepareDictionaryForMessage:(NSString *)message
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys:
            ...
            kVAORavenLogLevelArray[level], @"level",
            @"objc", @"platform",
            self.user ?: @"", @"user",
