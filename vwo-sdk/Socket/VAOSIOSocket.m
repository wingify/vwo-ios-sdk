...
@implementation VAOSIOSocket

// Generators
+ (void)socketWithHost:(NSString *)hostURL response:(void (^)(VAOSIOSocket *))response {
...
+ (void)socketWithHost:(NSString *)hostURL reconnectAutomatically:(BOOL)reconnectAutomatically attemptLimit:(NSInteger)attempts withDelay:(NSTimeInterval)reconnectionDelay maximumDelay:(NSTimeInterval)maximumDelay timeout:(NSTimeInterval)timeout response:(void (^)(VAOSIOSocket *))response {
    return [self socketWithHost: hostURL
         reconnectAutomatically: YES
                   attemptLimit: -1
                      withDelay: 1
...
+ (void)socketWithHost:(NSString *)hostURL reconnectAutomatically:(BOOL)reconnectAutomatically attemptLimit:(NSInteger)attempts withDelay:(NSTimeInterval)reconnectionDelay maximumDelay:(NSTimeInterval)maximumDelay timeout:(NSTimeInterval)timeout withTransports:(NSArray*)transports response:(void (^)(VAOSIOSocket *))response {
    ...
    [socket.javascriptContext setExceptionHandler: ^(JSContext *context, JSValue *errorValue) {
        [VAOLogger errorStr:[NSString stringWithFormat:@"JSError: %@", errorValue]];
    }];

    socket.javascriptContext[@"window"][@"onload"] = ^() {
        socket.thread = [NSThread currentThread];
        [socket.javascriptContext evaluateScript: vao_socket_io_js];
        [socket.javascriptContext evaluateScript: vao_blob_factory_js];
        
        NSString *socketConstructor = vao_socket_io_js_constructor(hostURL,
    ...
    for (id arg in args) {
        if ([arg isKindOfClass: [NSNull class]]) {
            ...
            if ([NSJSONSerialization isValidJSONObject:arg]) {
                ...
                [VAOLogger errorStr:[NSString stringWithFormat:@"SIOSocket serialization error at %@", NSStringFromSelector(_cmd)]];
            }
        }
    }
