...
#import "VWORavenClient_Private.h"
...
NSString *const kVWORavenLogLevelArray[] = {
    @"debug",
    @"info",
    @"warning",
    @"error",
...
NSString *const VWOuserDefaultsKey = @"nl.mixedCase.RavenClient.Exceptions.VWO";
NSString *const VWOsentryProtocol = @"4";
NSString *const VWOsentryClient = @"raven-objc/0.5.0";

static VWORavenClient *sharedClient = nil;
...
- (instancetype)initWithDSN:(NSString *)DSN extra:(NSDictionary *)extra tags:(NSDictionary *)tags logger:(NSString *)logger {
    ...
    if (self) {
        if (DSN)
        _config = [[VWORavenConfig alloc] init];
        _extra = extra;
        _logger = logger;
        self.tags = tags;
...
- (void)sendJSON:(NSData *)JSON {
    ...
    [[NSURLSession.sharedSession dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error) {
            VWOLogWarning(@"Sentry Connection failed! Error - %@", error.localizedDescription);
        }
    }] resume];
}
