...
@implementation VWOSIOSocket

// Generators
+ (void)socketWithHost:(NSString *)hostURL response:(void (^)(VWOSIOSocket *))response {
...
+ (void)socketWithHost:(NSString *)hostURL reconnectAutomatically:(BOOL)reconnectAutomatically attemptLimit:(NSInteger)attempts withDelay:(NSTimeInterval)reconnectionDelay maximumDelay:(NSTimeInterval)maximumDelay timeout:(NSTimeInterval)timeout response:(void (^)(VWOSIOSocket *))response {
    return [self socketWithHost: hostURL
         reconnectAutomatically: YES
                   attemptLimit: -1
                      withDelay: 1
...
+ (void)socketWithHost:(NSString *)hostURL reconnectAutomatically:(BOOL)reconnectAutomatically attemptLimit:(NSInteger)attempts withDelay:(NSTimeInterval)reconnectionDelay maximumDelay:(NSTimeInterval)maximumDelay timeout:(NSTimeInterval)timeout withTransports:(NSArray*)transports response:(void (^)(VWOSIOSocket *))response {
    ...
    [socket.javascriptContext setExceptionHandler: ^(JSContext *context, JSValue *errorValue) {
        VWOLogError(@"JSError: %@", errorValue);
    }];

    socket.javascriptContext[@"window"][@"onload"] = ^() {
        socket.thread = [NSThread currentThread];
        [socket.javascriptContext evaluateScript: vwo_socket_io_js];
        [socket.javascriptContext evaluateScript: vwo_blob_factory_js];
        
        NSString *socketConstructor = vwo_socket_io_js_constructor(hostURL,
    ...
    for (id arg in args) {
        if ([arg isKindOfClass: [NSNull class]]) {
            ...
            if ([NSJSONSerialization isValidJSONObject:arg]) {
                ...
                VWOLogError(@"Socket serialization error at %@", NSStringFromSelector(_cmd));
            }
        }
    }
